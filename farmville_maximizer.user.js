...
function Maximizer() {
	...
	this.init = function() {
		 ...
		 * @return	void
		 */
		function initFacebook(parent) {
			styles.addStyles(styles.getCommonStyles() + styles.getFacebookStyles());

			jQuery("#app_content_" + SCRIPT.appNo)
				.find('iframe[src*=farmville.com/flash.php]')
					.eq(0)
						.removeAttr('height')
						.removeAttr('width')
						.removeAttr('style')
	 ...
	 * @return	void
	 */
	this.createMenu = function(menuItems) {
		var self = this,
			$fvmmMenu = jQuery('<div id="fvmm_menu" />')
				.append(
	 ...
	 * @return	void
	 */
	this.createNoticebox = function() {
		var $notices = jQuery('.noticebox');
...
\n\
			...
			#fvmm_menu menu.show { display: block; }\n\
			#fvmm_menu li {\n\
				text-align: left;\n\
			}\n\
			#fvmm_menu a {\n\
...
\n\
			...
			#fvmm_notice .messages.show { display: block; }\n\
\n\
			#fvmm_noticebutton {\n\
				float: right;\n\
				height: 29px;\n\
...
function FarmvilleTool() {
	...
	this.init = function() {
		if (this.$panel == null) {
			var self = this,
				styles = new Styles();
			styles.addStyles(styles.getCalculatorStyles());
