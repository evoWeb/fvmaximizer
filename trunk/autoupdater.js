...
var AutoUpdater_73361 = {
    ...
    call: function(response) {
	    ...
	    onload: function(xpr) {AutoUpdater_73361.compare(xpr,response);}
        });
    },
    enable: function() {
        GM_registerMenuCommand("Enable "+this.name+" updates", function() {
            GM_setValue('updated_73361', new Date().getTime()+'');
            AutoUpdater_73361.call(true)
        });
    },
    ...
    check: function() {
        if (GM_getValue('updated_73361', 0) == "off")
            this.enable();
        else {
            if (+this.time > (+GM_getValue('updated_73361', 0) + 1000*60*60*24*this.days)) {
                GM_setValue('updated_73361', this.time+'');
                this.call();
            }
